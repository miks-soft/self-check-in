    ...
    @if(class_exists($options->model))
        ...
        @elseif($options->type == 'belongsToMany')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    $relationshipData = (isset($data)) ? $data : $dataTypeContent;
                    $selected_values = isset($relationshipData) ? $relationshipData->belongsToMany($options->model, $options->pivot_table, $options->foreign_pivot_key ?? null, $options->related_pivot_key ?? null, $options->parent_key ?? null, $options->key)->withPivot(...array_keys((array)$options->pivot_columns))->get()->map(function ($item, $key) use ($options) {
                        $label = $item->{$options->label};
                        $columns = (array)$options->pivot_columns;
                        if (count($columns) > 0)
                        {
                            $texts = [];
                            foreach ($columns as $column => $title) {
                                $texts[] = sprintf($title, $item->pivot->{$column});
                            }
                            $label .= ' ('. implode('; ', $texts).')';
                        }
            			return $label;
